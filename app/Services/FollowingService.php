...
//	}
        ...
        //フォローしたアカウントを登録している undifinedでエラーが発生することがあるため、定義されている場合のみ処理
        //または、すでにフォロー済みのアカウント等が対象となる（フォロー済みの場合、異なるjsonが返ってくるため、undifinedになる）
        if (isset($arr['screen_name'])) {
            Log::debug(print_r('定義されている', true));
            $followingtargets = new FollowingTarget;
            $followingtargets->target_id = $this->target_id;
            $followingtargets->following_name = $arr['screen_name'];
            $followingtargets->save();
        } else {
